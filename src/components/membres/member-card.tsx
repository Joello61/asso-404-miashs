          ...
          <div className="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400">
            ...
            {(linkedin || github || email) && (
              <div className="flex items-center">
                ...
                <span>
                  {[linkedin, github, email].filter(Boolean).length} lien
                  {[linkedin, github, email].filter(Boolean).length > 1
                    ? 's'
                    : ''}
                </span>
              </div>
...
export function MemberCardSkeleton({
  ...
  if (variant === 'mini') {
    return (
      <div className="flex items-center space-x-3 p-3">
        <div
          className={cn(
            'bg-slate-200 dark:bg-slate-700 rounded-full animate-pulse',
            currentConfig.avatarSize
          )}
        />
  ...
  return (
    <div
      className={cn(
        'bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 text-center',
        currentConfig.padding
      )}
    >
      {/* Avatar skeleton */}
